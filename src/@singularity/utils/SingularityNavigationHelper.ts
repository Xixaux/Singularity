...
class SingularityNavigationHelper {
	static selectById(nav: SingularityNavItemType[], id: string): SingularityNavItemType | undefined {
	...
	static filterNavigationByPermission(nav: SingularityNavItemType[], userRole: User['role']): SingularityNavItemType[] {
		return nav.reduce((acc: SingularityNavItemType[], item) => {
	 ...
	 */
	static removeNavItem(nav: SingularityNavItemType[], id: string): SingularityNavItemType[] {
		return nav.reduce((acc, node) => {
			...
			return acc;
		}, [] as SingularityNavItemType[]);
	}

	/**
	 * The updateNavItem function updates a navigation item by its ID with new data.
	 */
	static updateNavItem(nav: SingularityNavItemType[], id: string, item: PartialDeep<SingularityNavItemType>): SingularityNavItemType[] {
		return nav.map((node) => {
			if (node.id === id) {
	 ...
	 */
	static getFlatNavigation(navigationItems: SingularityNavItemType[] = [], flatNavigation = []) {
		for (const navItem of navigationItems) {
			if (navItem.type === 'item') {
				const _navtItem = SingularityNavItemModel(navItem);
		...
		return flatNavigation as SingularityNavItemType[] | [];
	}
	...
	static flattenNavigation(navigation: SingularityNavItemType[], parentOrder = ''): SingularityFlatNavItemType[] {
		if (!navigation) {
			return [];
		}
		...
		return navigation?.flatMap((item, index) => {
			...
			let flattened: SingularityFlatNavItemType[] = [
	...
	static unflattenNavigation(navigation: SingularityFlatNavItemType[]): SingularityNavItemType[] {
		const itemMap: Record<string, SingularityNavItemType> = {};
		navigation.forEach((item) => {
			itemMap[item.id] = { ...item, children: [] };
		});

		const rootItems: SingularityNavItemType[] = [];
...
export default SingularityNavigationHelper;
