...
/**
 * The SingularityRouteItemType type
 */
export type SingularityRouteItemType = {
	path?: string;
	element?: React.ReactNode;
	auth?: string[] | [];
	settings?: DeepPartial<SingularitySettingsConfigType>;
	children?: SingularityRouteItemType[];
};

/**
 * The SingularityRoutesType type is a custom type that is an array of SingularityRouteItemType objects.
 */
export type SingularityRoutesType = SingularityRouteItemType[];

/**
 * The SingularityRouteConfigType type is a custom type that defines the configuration for a set of routes.
 * It includes an optional routes property, an optional settings property, and an optional auth property.
 */
export type SingularityRouteConfigType = {
	routes: SingularityRoutesType;
	settings?: PartialDeep<SingularitySettingsConfigType>;
	auth?: string[] | [];
};

/**
 * The SingularityRouteConfigsType type is a custom type that is an array of SingularityRouteConfigType objects.
 */
export type SingularityRouteConfigsType = SingularityRouteConfigType[] | [];
...
/**
 * The SingularityUtils class provides utility functions for the Singularity project.
 */
class SingularityUtils {
	...
	/**
	 * The setRoutes function sets the routes for the Singularity project.
	 */
	static setRoutes(
		config: SingularityRouteConfigType,
		defaultAuth: SingularitySettingsConfigType['defaultAuth'] = undefined
	): SingularityRouteItemType[] {
		let routes: SingularityRouteItemType[] = [];

		if (config?.routes) {
			routes = [...config.routes];
		}

		const applyAuth = (route: SingularityRouteItemType, parentAuth: string[] | null) => {
	...
	static generateRoutesFromConfigs(
		configs: SingularityRouteConfigsType,
		defaultAuth: SingularitySettingsConfigType['defaultAuth']
	) {
		let allRoutes: SingularityRouteItemType[] = [];
		configs.forEach((config: SingularityRouteConfigType) => {
...
export default SingularityUtils;
