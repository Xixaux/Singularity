    ...
    Credentials({
        async authorize(formInput) {
            ...
            if (formInput.formType === 'signin') {
                try {
                    // Fetch the user data from your mock API using the provided email
                    const userResponse = await authGetDbUserByEmail(formInput.email);
                    const user = await userResponse.json();
    
                    // Check if the user exists and the passwords match
                    if (user && user.password === formInput.password) {
                        // Return the user object, which NextAuth will use to create the session
                        return {
                            email: user.email,
                            id: user.id,
                ...
                // If a user is not found, or the password doesn't match, or an error occurs, return null
                return null;
            }
