...
const IconOverlay = styled(Box)(({ theme }) => ({
  ...
  left: -30,
  opacity: 0.05,
  color: theme.palette.custom?.issues?.main ?? '#6200EA',
...
function IssuesWidget() {
  ...
  const accentIssueColor = theme.palette.custom?.issues?.main || '#6200EA';
  ...
  if (!widgets || !widgets.issues || !widget?.data || !widget?.title) {
    return (
      <WidgetContainer sx={{ height: 250 }}>
    ...
    >
      ...
      <Box display="flex" alignItems="center" justifyContent="space-between" mb={1} sx={{ zIndex: 1 }}>
        ...
        <Tooltip title="View Details">
          <IconButton aria-label="more" size="small" sx={{ color: secondarySubdued }} onClick={handleActionClick}>
      ...
      <ProgressWrapper>
        ...
        <Box position="absolute" display="flex" flexDirection="column" alignItems="center" justifyContent="center">
          ...
          <Typography variant="body2" color={secondarySubdued} mt={0.5} sx={{ textTransform: 'uppercase' }}>
            {data.name ?? 'ISSUES OPEN'}
          </Typography>
        </Box>
      </ProgressWrapper>

      {/* REMOVED GRAY BACKGROUND — now fully transparent / matches card */}
      <Box display="flex" justifyContent="center" alignItems="center" mt={2}>
        <SingularitySvgIcon size={16} sx={{ mr: 1, color: accentIssueColor }}>
          heroicons-outline:folder-open
        </SingularitySvgIcon>
        <Typography variant="body2" color={secondarySubdued} fontWeight="regular">
          {data.extra?.name ?? 'Unassigned'}:
        </Typography>
